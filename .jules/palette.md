...
## 2024-05-28 - Add Loading States and Tooltips for Preview Buttons
**Learning:** During testing, the TUI application's preview and graph rendering actions ("Browser Preview", "Render Graphs") had no loading states to indicate async processing, and buttons were lacking tooltips. In Textual background workers where direct access is tricky, safe UI mutation `self.call_from_thread` should be used with `try/except` wrapped functions to prevent potential race condition crashes where buttons might be out of tree.
**Action:** Implemented `loading` state to "Browser Preview" and "Render Graphs" buttons while they use Playwright to spin up background tabs, returning them to normal via `finally:` and `call_from_thread`. Also added proper accessibility tooltips to all preview controls.
