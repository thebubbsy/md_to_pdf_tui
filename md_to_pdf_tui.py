...
"""
            ...
            with TabbedContent():
                ...
                with TabPane("Paste & Preview"):
                    ...
                    with Horizontal(id="preview-controls"):
                         yield Button("👁️ TUI Preview", id="toggle-view-btn", disabled=True, variant="primary", tooltip="Preview markdown with Textual UI")
                         yield Button("🌐 Browser Preview", id="browser-preview-btn", variant="default", tooltip="Open a high-fidelity preview in your web browser")
                         if HAS_PIXELS:
                             yield Button("🖼️ Render Graphs", id="tui-render-btn", variant="default", tooltip="Render Mermaid diagrams inline (may be slow)")
        ...
        def on_switch_changed(self, event: Switch.Changed):
            if event.switch.id == "a4-width-switch": self.settings["a4_fixed_width"] = event.value
                ...
                if event.value:
                    ...
                    toggle_btn.tooltip = "Preview markdown with Textual UI"
                else:
                    # File Mode
        ...
        async def on_button_pressed(self, event: Button.Pressed):
            ...
            if event.button.id == "convert-btn": self.action_convert()
                ...
                if switcher.current == "paste-area":
                    ...
                    btn.variant = "primary"
                    btn.tooltip = "Preview markdown with Textual UI"
        ...
        def action_render_tui(self):
            ...
            if self.use_paste_source:
                ...
                switcher.current = "md-view"
                toggle_btn = self.query_one("#toggle-view-btn", Button)
                toggle_btn.label = "✏️ Back to Edit"
                toggle_btn.variant = "default"
                toggle_btn.tooltip = "Return to markdown editor"
