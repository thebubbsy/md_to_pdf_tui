    ...
    class HelpScreen(ModalScreen):
        ...
        def compose(self) -> ComposeResult:
            with Container(id="help-dialog"):
                ...
                yield Rule()
                yield Button("Close", variant="primary", id="dismiss-btn", tooltip="Close this help dialog")
            yield Footer()
...
"""
            ...
            with TabbedContent():
                ...
                with TabPane("Paste & Preview"):
                    ...
                    with Horizontal(id="preview-controls"):
                         yield Button("👁️ TUI Preview", id="toggle-view-btn", disabled=True, variant="primary", tooltip="Switch to terminal preview")
                         yield Button("🌐 Browser Preview", id="browser-preview-btn", variant="default", tooltip="Open HTML preview in default web browser")
                         if HAS_PIXELS:
                             yield Button("🖼️ Render Graphs", id="tui-render-btn", variant="default", tooltip="Render Mermaid diagrams in the terminal")
        ...
        def on_switch_changed(self, event: Switch.Changed):
            if event.switch.id == "a4-width-switch": self.settings["a4_fixed_width"] = event.value
                ...
                if event.value:
                    ...
                    toggle_btn.tooltip = "Switch to terminal preview"
                else:
                    # File Mode
        ...
        async def on_button_pressed(self, event: Button.Pressed):
            ...
            if event.button.id == "convert-btn": self.action_convert()
                ...
                if switcher.current == "paste-area":
                    ...
                    btn.variant = "default"
                    btn.tooltip = "Switch to text editor"
                else:
                    # Switch back to Edit
                    switcher.current = "paste-area"
                    btn.label = "👁️ TUI Preview"
                    btn.variant = "primary"
                    btn.tooltip = "Switch to terminal preview"
        ...
        def action_render_tui(self):
            ...
            if self.use_paste_source:
                switcher = self.query_one("#preview-switcher", ContentSwitcher)
                switcher.current = "md-view"
                btn = self.query_one("#toggle-view-btn", Button)
                btn.label = "✏️ Back to Edit"
                btn.variant = "default"
                btn.tooltip = "Switch to text editor"
