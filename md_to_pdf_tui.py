...
def process_resources(md_text: str, temp_dir: Path) -> str:
    ...
    # Optimization: Use set comprehensions and union for faster C-level iteration
    urls = {match.group(2) for match in MD_IMG_PATTERN.finditer(md_text)} | {match.group(1) for match in HTML_IMG_PATTERN.finditer(md_text)}
...
async def generate_docx_core(md_path: Path, docx_path: Path, log_fn=print, prog_fn=None, settings: dict=None) -> None:
    ...
    # --- PROCESS ALERTS ---
    # Fast-path optimization: bypass O(N) processing if there are no alerts
