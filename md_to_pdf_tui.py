    ...
    class HelpScreen(ModalScreen):
        ...
        def compose(self) -> ComposeResult:
            with Container(id="help-dialog"):
                ...
                yield Rule()
                yield Button("Close", variant="primary", id="dismiss-btn", tooltip="Close help dialog")
            yield Footer()
            ...
            with TabbedContent():
                ...
                with TabPane("Paste & Preview"):
                    ...
                    with Horizontal(id="preview-controls"):
                         yield Button("👁️ TUI Preview", id="toggle-view-btn", disabled=True, variant="primary", tooltip="Preview markdown in TUI (Disabled in File Mode)")
                         yield Button("🌐 Browser Preview", id="browser-preview-btn", variant="default", tooltip="Preview current document in a web browser")
                         if HAS_PIXELS:
        ...
        def on_switch_changed(self, event: Switch.Changed):
            if event.switch.id == "a4-width-switch": self.settings["a4_fixed_width"] = event.value
                ...
                if event.value:
                    ...
                    toggle_btn.tooltip = "Preview markdown in TUI"
                else:
                    # File Mode
                    switcher.current = "md-view"
                    toggle_btn.disabled = True
                    toggle_btn.tooltip = "Preview markdown in TUI (Disabled in File Mode)"
        ...
        async def on_button_pressed(self, event: Button.Pressed):
            ...
            if event.button.id == "convert-btn": self.action_convert()
                ...
                if switcher.current == "paste-area":
                    ...
                    btn.variant = "primary"
                    btn.tooltip = "Preview markdown in TUI"
