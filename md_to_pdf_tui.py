...
def process_resources(md_text: str, temp_dir: Path) -> str:
    ...
    # ⚡ Bolt: Using set comprehensions and union operator is faster than explicit for loops with set.add()
    urls = {m.group(2) for m in MD_IMG_PATTERN.finditer(md_text)} | {m.group(1) for m in HTML_IMG_PATTERN.finditer(md_text)}
