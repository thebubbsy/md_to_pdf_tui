...
"""
            ...
            with TabbedContent():
                ...
                with TabPane("Paste & Preview"):
                    ...
                    with Horizontal(id="preview-controls"):
                         yield Button("👁️ TUI Preview", id="toggle-view-btn", disabled=True, variant="primary", tooltip="Preview only available in Paste Mode")
                         yield Button("🌐 Browser Preview", id="browser-preview-btn", variant="default", tooltip="Preview Markdown in default web browser")
                         if HAS_PIXELS:
                             yield Button("🖼️ Render Graphs", id="tui-render-btn", variant="default", tooltip="Render Mermaid diagrams in terminal preview")
        ...
        def on_switch_changed(self, event: Switch.Changed):
            if event.switch.id == "a4-width-switch": self.settings["a4_fixed_width"] = event.value
                ...
                if event.value:
                    ...
                    toggle_btn.tooltip = "Preview Markdown in terminal"
                else:
                    # File Mode
                    switcher.current = "md-view"
                    toggle_btn.disabled = True
                    toggle_btn.tooltip = "Preview only available in Paste Mode"
        ...
        async def on_button_pressed(self, event: Button.Pressed):
            ...
            if event.button.id == "convert-btn": self.action_convert()
                ...
                if switcher.current == "paste-area":
                    ...
                    btn.variant = "default"
                    btn.tooltip = "Return to markdown editor"
                else:
                    # Switch back to Edit
                    switcher.current = "paste-area"
                    btn.label = "👁️ TUI Preview"
                    btn.variant = "primary"
                    btn.tooltip = "Preview Markdown in terminal"
        ...
        def action_render_tui(self):
            ...
            if self.use_paste_source:
                ...
                self.query_one("#toggle-view-btn", Button).tooltip = "Return to markdown editor"
