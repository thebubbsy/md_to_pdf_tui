            ...
            with TabbedContent():
                ...
                with TabPane("Paste & Preview"):
                    ...
                    with Horizontal(id="preview-controls"):
                         yield Button("👁️ TUI Preview", id="toggle-view-btn", disabled=True, variant="primary", tooltip="Preview not available in File Mode")
                         yield Button("🌐 Browser Preview", id="browser-preview-btn", variant="default", tooltip="Preview markdown in system web browser")
                         if HAS_PIXELS:
        ...
        def on_switch_changed(self, event: Switch.Changed):
            if event.switch.id == "a4-width-switch": self.settings["a4_fixed_width"] = event.value
                ...
                if event.value:
                    ...
                    toggle_btn.tooltip = "Switch to preview mode"
                else:
                    # File Mode
                    switcher.current = "md-view"
                    toggle_btn.disabled = True
                    toggle_btn.tooltip = "Preview not available in File Mode"
        ...
        async def on_button_pressed(self, event: Button.Pressed):
            ...
            if event.button.id == "convert-btn": self.action_convert()
                ...
                if switcher.current == "paste-area":
                    ...
                    btn.variant = "primary"
                    btn.tooltip = "Switch to preview mode"
