...
def create_html_content(md_text: str, settings: dict) -> str:
    ...
    # ⚡ Bolt: Conditionally inject Mermaid.js only when the document contains mermaid
    # This prevents loading a large JS library for documents without diagrams, speeding up rendering.
    mermaid_script = ""
    if "mermaid" in body.lower():
        mermaid_script = f'''<script src="https://cdn.jsdelivr.net/npm/mermaid@11.4.1/dist/mermaid.min.js"></script>
<script>
mermaid.initialize({{ 
    startOnLoad: true, 
    {m_theme_init},
...
async def generate_docx_core(md_path: Path, docx_path: Path, log_fn=print, prog_fn=None, settings: dict=None, browser=None) -> None:
    ...
    # --- PROCESS ALERTS ---
    # ⚡ Bolt: Fast-path optimization. Bypass expensive line-by-line processing
    # if the document clearly contains no alerts.
...
"""
            ...
            with TabbedContent():
                ...
                with TabPane("Paste & Preview"):
                    ...
                    with Horizontal(id="preview-controls"):
                         ...
                         yield Button("🌐 Browser Preview", id="browser-preview-btn", variant="default", tooltip="Open a temporary HTML preview in your default browser")
                         if HAS_PIXELS:
                             yield Button("🖼️ Render Graphs", id="tui-render-btn", variant="default", tooltip="Render and display Mermaid diagrams directly in the terminal")
        ...
        def on_switch_changed(self, event: Switch.Changed):
            if event.switch.id == "a4-width-switch": self.settings["a4_fixed_width"] = event.value
                ...
                if event.value:
                    ...
                    toggle_btn.tooltip = "Preview the rendered markdown"
        ...
        async def on_button_pressed(self, event: Button.Pressed):
            ...
            if event.button.id == "convert-btn": self.action_convert()
                ...
                if switcher.current == "paste-area":
                    ...
                    btn.tooltip = "Return to the markdown editor"
                    btn.variant = "default"
                else:
                    # Switch back to Edit
                    switcher.current = "paste-area"
                    btn.label = "👁️ TUI Preview"
                    btn.tooltip = "Preview the rendered markdown"
                    btn.variant = "primary"
        ...
        def action_render_tui(self):
            ...
            if self.use_paste_source:
                ...
                self.query_one("#toggle-view-btn", Button).tooltip = "Return to the markdown editor"
