...
async def generate_docx_core(md_path: Path, docx_path: Path, log_fn=print, prog_fn=None, settings: dict=None) -> None:
    ...
    # --- PROCESS ALERTS ---
    # ⚡ Bolt: Fast-path optimization to skip line-by-line split and processing
    # if the document clearly doesn't contain any alert blocks.
    # We do a cheap substring check first, followed by a regex check.
